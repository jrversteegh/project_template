...
po::variables_map const& get_program_options();
...
void print_usage(po::options_description& options_description) {
  std::cout << "Usage: PROJECT [<options>] command" << std::endl;
  std::cout << std::endl;
  std::cout << options_description << std::endl;
}

inline void print_version() {
  std::cout << "PROJECT " << STRINGIFY(VERSION) << std::endl;
  ...
  std::cout << "Written by AUTHOR <EMAIL>" << std::endl;
}
...
int main(int argc, char* argv[]) {
  try {
    ...
    desc_args.add_options()("help,h", "display this help and exit")(
        "version,v", "display version info and exit");

    po::options_description command_args{"Command"};
    command_args.add_options()("command", po::value<std::vector<std::string>>(),
                               "command");
